...
def vertical_velocity(u,h,mesh):
    """
    ...
    ω_expr = -(u[0].dx(0) + u[1].dx(1)) * ζ
    return ω_expr



class slopes(object):
    def solve(self, u, h, mesh):
        """
        ...
        """
        V = u.function_space()
        w_expr = orthogonal_velocity(u,h,mesh)
        slopes = firedrake.interpolate(firedrake.as_vector((firedrake.atan(w_expr/u[0]),firedrake.atan(w_expr/u[1]))),V)
        return slopes
